                  ...
                  <div className="flex items-center gap-2">
                    {t.status === "todo" && (
                      <button onClick={() => setStatus(t.id, "in_progress")} disabled={updateStatus.isPending}
                        className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-amber-50 text-amber-600 border border-amber-100 hover:bg-amber-100 transition-all disabled:opacity-50">
                        <Play size={13} /> Boshlash
                      </button>
                    )}
                    {t.status === "in_progress" && (
                      <button onClick={() => setStatus(t.id, "done")} disabled={updateStatus.isPending}
                        className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-600 border border-emerald-100 hover:bg-emerald-100 transition-all disabled:opacity-50">
                        <CheckCircle size={13} /> Bajarildi
                      </button>
                    )}
                    {t.status === "done" && (
                      <>
                        <span className="inline-flex items-center gap-1.5 text-xs text-emerald-600 font-bold uppercase tracking-wider">
                          <CheckCircle size={13} /> Yakunlandi
                        </span>
                        <button onClick={() => setStatus(t.id, "in_progress")} disabled={updateStatus.isPending}
                          className="inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-all disabled:opacity-50">
